...
function initAddressAutocomplete() {
    $(document).ready(function() {
        ...
        $("#en__field_supporter_NOT_TAGGED_2").attr("autocomplete","nope");
        $(".en__field--country").removeClass("en__hidden");
          MakeVisibleFields("hide");

        // Keyup / input handler
        $(document).on("input", "#en__field_supporter_NOT_TAGGED_2", function() {
            $('.en__field--NOT_TAGGED_2 .en__field__error').remove();
            $('.en__field--NOT_TAGGED_2').removeClass('en__field--validationFailed');  

            $('.en__field--region').find('.en__field__element--select .select-items div').removeClass('same-as-selected');
            $('.en__field--region').find('.en__field__element--select .select-selected').text("Select State");
            $('.en__field--country').find('.en__field__element--select .select-items div').removeClass('same-as-selected');
            $('.en__field--country').find('.en__field__element--select .select-selected').text("Select Country");
        ...
        function MakeVisibleFields(swapFlag){
            ...
            if(swapFlag === "hide") {
                $(".en__field--country, .en__field--region").addClass("en__field--hidden");
            } else {
                $(".en__field--country, .en__field--region").removeClass("en__field--hidden");
            }
        }
        ...
        const googleAutocomplete = {
            autocompleteField: function(fieldId) {
                ...
                google.maps.event.addListener(autocomplete, "place_changed", function () {
                    ...
                    let hasStreet = false, hasPostalCode = false;
                    ...
                    address.forEach(component => {
                        ...
                        switch(type) {
                            case "country":
                                    ...
                                    $('.en__field--country .select-selected').text(component.long_name);
                                
                                    $('.en__field--country .select-items div:contains("' + component.long_name + '")')
                            ...
                            //     break;
                             ...
                             case "administrative_area_level_1":
                                    ...
                                    $('.en__field--region .select-selected').text(component.long_name);
                                    $('.en__field--region .select-items div:contains("' + component.long_name + '")')
                                        .addClass('same-as-selected')
                                        .click();
                            ...
                            case "administrative_area_level_2":
                                ...
                                $("#en__field_supporter_secondRegion").val(component.long_name).closest('.en__field').addClass("en__field--hidden");
                                break;
                            
                            case "locality":
                                locationInfo.city = component.long_name;
                                $("#en__field_supporter_city").val(component.long_name).closest('.en__field').addClass("en__field--hidden");
                                break;
                             case "postal_town":
                                locationInfo.city = component.long_name;
                                $("#en__field_supporter_city").val(component.long_name).closest('.en__field').addClass("en__field--hidden");
                            ...
                            case "route":
                                ...
                                $("#en__field_supporter_address1").val(streetValue + " " + component.long_name).closest('.en__field').addClass("en__field--hidden");
                                break;
                            case "subpremise":
                                locationInfo.streettwo = component.long_name;
                                $("#en__field_supporter_address2").val(component.long_name).closest('.en__field').addClass("en__field--hidden");
                                break;
                            case "street_number":
                                locationInfo.streetNumber = component.long_name;
                                let streetNum = $("#en__field_supporter_address1").val();
                                $("#en__field_supporter_address1").val(component.long_name + " " + streetNum);
                                break;
                            default:
                                break;
                        }
                    ...
                    // Civic API call
                    const address_string = $("#en__field_supporter_NOT_TAGGED_2").val();
                    const civicFields = {
                    ...
                    $.ajax({
                        ...
                        method: "GET",
                        data: { address: address_string, alt: "json", includeOffices: false, key: "AIzaSyB1iSGfBn28MUaqCQ3VhJ9Rrhc81aT_1yQ" },
                            ...
                            Object.keys(civicFields).forEach(function(key) {
                                Object.keys(divisions).forEach(function(divkey) {
                                    if(divkey.indexOf(key) != -1){
                                        $(civicFields[key]).val(divisions[divkey].name).closest('.en__field').addClass("en__field--hidden");
                                    }
                                });
                            });
                            Object.keys(divisions).forEach(function(divkey) {
                                if(divkey.indexOf("/county") != -1 && divkey.indexOf("council_district") == -1){
                                    $("#en__field_supporter_NOT_TAGGED_49").val(divisions[divkey].name).closest('.en__field').addClass("en__field--hidden");
                                }
                            });
                        }
                    });
        ...
        googleAutocomplete.autocompleteField("en__field_supporter_NOT_TAGGED_2");
        ...
        $("#en__field_supporter_NOT_TAGGED_2").focusout(function() {
            setTimeout(function(){
                const fieldVal = $("#en__field_supporter_NOT_TAGGED_2").val();
                if(fieldVal == "") {
                    MakeVisibleFields("hide");
                } else {
                    ...
                    if(street == "" || postcode == "") {
                         $('.en__field--NOT_TAGGED_2').removeClass('en__field--validationFailed'); 
                        if(!$(".en__field--NOT_TAGGED_2").hasClass("en__field--validationFailed")) {
                            $('.en__field--NOT_TAGGED_2').append('<div class="en__field__error">Please select a valid drop down address</div>').addClass('en__field--validationFailed');
                        }
                    }
                }
            }, 800);
      ...
      setTimeout(function () {
        if ($(".en__field--validationFailed").length > 0) {
          ...
          setTimeout(function () {
            ...
            $('.en__component .en__component--formblock .active').css('display', 'block');
          }, 100);
        }
      }, 1000);
    }
    ...
    inputFields.forEach(function (field) {
        if (field.name) {
          if (field.name.startsWith("supporter.")) {
            var supporterFieldName = field.name.replace("supporter.", "supporter_");
...
window.enOnValidate = function () {
    return new Promise((resolve, reject) => {
        const addressField = $('#en__field_supporter_NOT_TAGGED_2').closest('.en__field');
...
document.addEventListener("DOMContentLoaded", function () {
    const inputField = document.getElementById("en__field_supporter_emailAddress");
...
$( "#en__field_supporter_postcode" ).keyup(function(e) {
    ...
    $('.en__field--region').find('.en__field__element--select').find('.select-selected').text("Select State");
    var divs = $('.en__field--region').find('.en__field__element--select').find('.select-items').find('div').removeClass('same-as-selected');
    ...
    if(postCode.length==5){
        $.ajax({
            ...
            beforeSend: function(request) {
                request.setRequestHeader("x-key", "6b0dcc1af27a260624af140faa57d34caf51ab55");
            },
            ...
            success: function(data) {
                ...
                $('.en__field--region').find('.en__field__element--select').find('.select-selected').text(data[0].state);
                var divs = $('.en__field--region').find('.en__field__element--select').find('.select-items').find('div:contains("'+data[0].state+'")').addClass('same-as-selected');
