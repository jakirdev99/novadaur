...
function initCustomDropdowns() {
  ...
  $(".en__field__element--select").each(function () {

    const container = $(this);
    const select = container.find("select")[0];

    // Skip region dropdown
    if (!select || select.id === "en__field_supporter_region") {
      return;
    }
    ...
    const selected = $("<div>", {
      ...
      html: select.options[select.selectedIndex].text
    });
...
function syncDropdowns() {
  ...
  $(".en__field__element--select select").each(function () {
